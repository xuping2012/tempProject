...
#         self.conf=HandleConfig(config.conf_path+"common.conf")
        u'''连接池方式'''
        self.conn = HandleMySQL.__getConnect()#self.db_info
...
if __name__ == "__main__":
    conn=HandleMySQL()
    result1=conn.mysql_getrows("select * from futureloan.member where mobile_phone='13266515341' ")
    res=conn.count("select * from futureloan.member where mobile_phone='13266515341' ")
    print("sql查询一行所有结果：{},\n查询结果：{}".format(result1,res))
    result=conn.get_one("select leave_amount from futureloan.member where mobile_phone='13266515341'")
    print("sql查询某个字段结果：{}".format(result))
    res1=conn.get_one("SELECT leave_amount FROM futureloan.member WHERE mobile_phone=15512345678")
    print(type(res1))
    conn.mysql_close()